...
public final class ExcelImportUtil {
     ...
     * @throws Exception
     */
    public static <T> List<T> importExcelBysheet(InputStream inputstream, Class<?> pojoClass, ImportParams params,String sheetName) throws Exception {
        return new ExcelImportServer().importExcelBySheetName(inputstream, pojoClass, params,sheetName).getList();
    }
    /**
     ...
     * @throws Exception
     */
    public static <T> ExcelImportResult<T> importExcelVerify(File file, Class<?> pojoClass, ImportParams params) throws Exception{
        return importExcelVerify(file, pojoClass, params, true);
    }
     ...
     * @throws Exception
     */
    public static <T> ExcelImportResult<T> importExcelVerify(File file, Class<?> pojoClass, Class<?> checkClass, ImportParams params) throws Exception{
        return importExcelVerify(file, pojoClass, checkClass, params, true);
    }

    public static <T> ExcelImportResult<T> importExcelVerify(File file, Class<?> pojoClass, ImportParams params, boolean titleCheck) throws Exception{
        return importExcelVerify(file, pojoClass, pojoClass, params, titleCheck);
    }

    public static <T> ExcelImportResult<T> importExcelVerify(File file, Class<?> pojoClass, Class<?> checkClass, ImportParams params, boolean titleCheck) throws Exception{
        FileInputStream in = null;
        try {
            in = new FileInputStream(file);
            if (titleCheck) {
                boolean checkResult = ExcelImportCheckUtil.check(new FileInputStream(file), checkClass, params, 1.0);
                if (!checkResult) {
                    throw new RuntimeException("导入文件表头与模板文件不符，请下载导入模板文件进行导入！");
                }
            }
