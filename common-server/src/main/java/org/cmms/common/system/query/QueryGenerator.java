package org.cmms.common.system.query;
...
public class QueryGenerator {
	...
	public static void installMplus(QueryWrapper<?> queryWrapper,Object searchObj,Map<String, String[]> parameterMap) {
		...
		*/
		...
		Field[] fields = oConvertUtils.getAllFields(searchObj);
		...
		for (int i = 0; i < origDescriptors.length; i++) {
			...
			try {
				if (judgedIsUselessField(name)|| !PropertyUtils.isReadable(searchObj, name) || judgedIsNotTableField(name, fields)) {
					continue;
				}
...
//						});
	 ...
	 * @return
	 */
	private static boolean judgedIsNotTableField(String name, Field[] fields) {
		for (Field field : fields) {
			if (name.equals(field.getName())) {
				if (field.getAnnotation(TableField.class) != null) {
					boolean exist =  field.getAnnotation(TableField.class).exist();
					if(!exist) {
						return true;
					}
				}
