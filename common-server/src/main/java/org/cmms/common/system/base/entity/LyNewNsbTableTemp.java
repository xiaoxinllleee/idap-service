 ...
 * @Created by eran
 */
public class LyNewNsbTableTemp {
    private HSSFWorkbook workbook;

    private HSSFSheet sheet;

    public LyNewNsbTableTemp(HSSFWorkbook workbook, HSSFSheet sheet) {
    ...
    public void setTableName() {
        ...
        this.createCellRange(sheet, 0, 0, 0, 12, true);
        HSSFRow row = sheet.createRow(0);
        this.createCell(sheet, cellStyle, row, 0, "浏阳农商行沙市支行农户评级授信年审表", false);
    }

    public void setRowTwoNam() {
        Font fontStyle = workbook.createFont();
        fontStyle.setFontName("宋体");
        ...
        this.createCell(sheet, cellStyle, row, 10, "单位：万元", false);
        //this.createCellRange(sheet, 1, 1, 0, 0, true);
        //this.createCellRange(sheet, 1, 1, 1, 16, true);
        this.createCellRange(sheet, 1, 1, 10, 12, false);
    }
    ...
    public void setTableFooter() {
        ...
        row.createCell(0).setCellValue(new HSSFRichTextString("初评小组意见：同意"));
        CellUtil.getCell(row, 0).setCellStyle(cellStyle);
        row.createCell(4).setCellValue(new HSSFRichTextString("复评小组意见：同意"));
        CellUtil.getCell(row, 4).setCellStyle(cellStyle);
        row.createCell(8).setCellValue(new HSSFRichTextString("支行待审小组意见：同意"));
        CellUtil.getCell(row, 8).setCellStyle(cellStyle);


        this.createCellRange(sheet, rowIndex, rowIndex, 0, 3, true);
        this.createCellRange(sheet, rowIndex, rowIndex, 4, 7, true);
        this.createCellRange(sheet, rowIndex, rowIndex, 8, 12, true);
        ...
        sheet.setDefaultColumnStyle(8, cs);

    }


    public void setTableFooter2() {
        int lastRowNum = (sheet.getLastRowNum());
        int rowIndex = lastRowNum + 1;
        ...
        row.createCell(0).setCellValue(new HSSFRichTextString("初评小组签字："));
        CellUtil.getCell(row, 0).setCellStyle(cellStyle);
        row.createCell(4).setCellValue(new HSSFRichTextString("复评小组签字："));
        CellUtil.getCell(row, 4).setCellStyle(cellStyle);
        row.createCell(8).setCellValue(new HSSFRichTextString("支行待审小组签字："));
        CellUtil.getCell(row, 8).setCellStyle(cellStyle);
