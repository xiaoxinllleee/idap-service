        ...
        for (int i = 0; i < params.getSheetNum(); i++) {
            ...
            while (rows.hasNext() && (row == null || sheet.getLastRowNum() - row.getRowNum() > params.getLastOfInvalidRow())) {
                ...
                try {
                    ...
                    //优化 导入文件中可以比实体类中多字段
                    //遍历实体类中的所有字段，是否在导入文件中有
                    for (Map.Entry<String, ExcelImportEntity> entry : excelParams.entrySet()) {
                        String mapKey = entry.getKey();
                        ExcelImportEntity mapValue = entry.getValue();
                        if(titlemap.containsValue(mapKey) || Map.class.equals(pojoClass)) {
                            successNum+=1;
                        } else {
                            errorNum+=1;
                        }
                    ...
                    if(excelCollection.size()>0){
                        ...
                        Map<String, ExcelImportEntity> collectionExcelParams = param.getExcelParams();
                        //遍历列表中的所有字段
                        for (Map.Entry<String, ExcelImportEntity> entry : collectionExcelParams.entrySet()) {
                            String mapKey = entry.getKey();
                            ExcelImportEntity mapValue = entry.getValue();
