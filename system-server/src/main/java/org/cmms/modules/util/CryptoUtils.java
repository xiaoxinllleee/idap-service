...
import org.cmms.modules.util.CryptoUtils.Algorithm.Encryption;
import org.cmms.modules.util.CryptoUtils.Algorithm.Signing;
...
public class CryptoUtils {
     ...
     * 对称加密
     * @param secretKey 密钥
     *  iv 加密向量，只有CBC模式才支持，如果是CBC则必传
     * @param plainText 明文
     *  algorithm 对称加密算法，如AES、DES
     * @return
     * @throws Exception
     */
    public static String encryptAES(String secretKey, String plainText) throws Exception {
        SecretKey key = decodeSymmetricKey(secretKey, Encryption.AES_ECB_PKCS5);
        byte[] plainTextInBytes = plainText.getBytes(DEFAULT_CHARSET);
        byte[] ciphertextInBytes = transform(Encryption.AES_ECB_PKCS5, Cipher.ENCRYPT_MODE, key, null, plainTextInBytes);
     ...
     * 对称解密
     * @param secretKey 密钥
     * 加密向量，只有CBC模式才支持，如果是CBC则必传
     * @param ciphertext 密文
     * 对称加密算法，如AES、DES
     * @return
     * @throws Exception
     */
    public static String decryptAES(String secretKey,String ciphertext) throws Exception {
        SecretKey key = decodeSymmetricKey(secretKey, Encryption.AES_ECB_PKCS5);
        byte[] ciphertextInBytes = BASE64_DECODER.decode(ciphertext);
        byte[] plainTextInBytes = transform(Encryption.AES_ECB_PKCS5, Cipher.DECRYPT_MODE, key, null, ciphertextInBytes);
        return new String(plainTextInBytes, DEFAULT_CHARSET);
    }
    ...
    public static String createUUID(){
        Date dg = DateUtil.getDate("15821012000000");
        Date dc = new Date();
        long t = dc.getTime() - dg.getTime();
        System.out.println("t="+t);
        String tl = getIntegerBits(t,0,31);
        String tm = getIntegerBits(t,32,47);
        String thv = getIntegerBits(t,48,59) + '1'; // version 1, security version is 2
    ...
    public static String getIntegerBits(long val,int start,int end){
        String base16 = returnBase(val);
        String[] quadArray =new String[16];
        String quadString = "";
        int i = 0;
        for(i=0;i<base16.length();i++){
            quadArray[i]=base16.substring(i,i+1);
        }
        for(i= (int) Math.floor(start/4); i<=Math.floor(end/4); i++){
            if(quadArray[i]!=null || quadArray[i] == "") {
                quadString += '0';
            }else {
    ...
    public static String returnBase(long number){
        String base = Integer.toHexString((int) number).toUpperCase();
        return base;
    }

    public static void main(String[] args) {
        System.out.println(URLEncoder.encode("6ezImEfyeTbqqjdOtv++2oBbPFNhiacHr6+8lUYbY64="));

        String secretKey = "B70E4921CD3AF568";
        //6fad20b1186b4a8bb2359462438a9eec
        //b697865c61ca42d29aca8c8676beee7c
        String cipherText = "yV9k3fQozhvWcGaTTXITYLJfybLH1APD4V295sMGeKGaKAHBz9b/oFFcPmmrfydB96aVnO2Wonixp9da7KptWUmYkqWNOvtQrzcoJWdbDW8vNihZRbxBrjz+E7ciDSUDlASzdhav0+HkW/cDymKRUi5ysGT11gee1UNpp2616+k=";
//{"password":"MTIzNDU2Nzg=","code":"1","imei":"","isAndroid":"0","uuid":"46f5b77097fb45f4bb51a344141f9e80","username":"3650065"
//{"password":"MTIzNDU2Nzg=","code":"2","imei":"","isAndroid":"0","uuid":"6fad20b1186b4a8bb2359462438a9eec","username":"3650065"}
//{"password":"MTIzNDU2Nzg=","code":"1","imei":"","isAndroid":"0","uuid":"72cf87293e6f4565b75bd8f8ac321f3a","username":"3650065"}
//{"password":"MTIzNDU2Nzg=","code":"2","imei":"","isAndroid":"0","uuid":"6fad20b1186b4a8bb2359462438a9eec","username":"3650065"}
        try {
            String jiemizfc = CryptoUtils.decryptAES(secretKey, cipherText);
            System.out.println("解密字符串：" + cipherText);
            System.out.println("解密后的明文为：" + jiemizfc);
            JSONObject parse = (JSONObject) JSON.parse(jiemizfc);

           // byte[] ivInBytes = BASE64_DECODER.decode(img);
            //System.out.println("解密后的明文为：" + ivInBytes.toString());

            String jmzfc ="{\"msg\":\"验证码错误\",\"code\":500}";
            String jmtext = CryptoUtils.encryptAES(secretKey, jmzfc);

            JSONObject jsonObject=new JSONObject(new LinkedHashMap());
            jsonObject.put("username","3650065");
            jsonObject.put("password","MTIzNDU2Nzg=");
            jsonObject.put("code","6");
            jsonObject.put("uuid","f84ae6b8e092439e9f8b77b43db0ef7a");
            jsonObject.put("isAndroid","0");
            jsonObject.put("imei","");

            System.out.println("加密前的字符串：" + jiemizfc);
            String postString = CryptoUtils.encryptAES("B70E4921CD3AF568", jsonObject.toJSONString());
            System.out.println("加密后的字符串：" + postString);
