...
public class SysUserDepartServiceImpl extends ServiceImpl<SysUserDepartMapper, SysUserDepart> implements ISysUserDepartService {
	...
	@Autowired
	private IHrBasOrganizationService sysHrDepartService;
	@Autowired
	...
	@Override
	public List<DepartIdHrModel> queryHrDepartIdsOfUser(String userId) {
		LambdaQueryWrapper<SysUserDepart> queryUDep = new LambdaQueryWrapper<SysUserDepart>();
		LambdaQueryWrapper<HrBasOrganization> queryDep = new LambdaQueryWrapper<HrBasOrganization>();
		try {
			queryUDep.eq(SysUserDepart::getUserId, userId);
			List<String> depIdList = new ArrayList<>();
			List<DepartIdHrModel> depIdModelList = new ArrayList<>();
			List<SysUserDepart> userDepList = this.list(queryUDep);
			if(userDepList != null && userDepList.size() > 0) {
				for(SysUserDepart userDepart : userDepList) {
					depIdList.add(userDepart.getDepId());
				}
				queryDep.in(HrBasOrganization::getZzbz, depIdList);
				List<HrBasOrganization> depList = sysHrDepartService.list(queryDep);
				if(depList != null || depList.size() > 0) {
					for(HrBasOrganization depart : depList) {
						depIdModelList.add(new DepartIdHrModel().convertByUserDepart(depart));
					}
				}
				return depIdModelList;
			}
