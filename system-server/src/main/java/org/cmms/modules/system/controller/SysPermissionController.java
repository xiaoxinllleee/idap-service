...
import org.cmms.modules.system.model.SysPermissionTree;
import org.cmms.modules.system.model.SysRoleDeisgnModel;
import org.cmms.modules.system.model.TreeModel;
import org.cmms.modules.system.service.*;
import org.cmms.modules.system.util.PermissionDataUtil;
...
public class SysPermissionController {
	...
	public Result<List<SysPermissionTree>> list(@RequestParam("subSystemId") String subSystemId) {
        long start = System.currentTimeMillis();
		Result<List<SysPermissionTree>> result = new Result<>();
		try {
			LambdaQueryWrapper<SysPermission> query = new LambdaQueryWrapper<SysPermission>();
			query.eq(SysPermission::getDelFlag, CommonConstant.DEL_FLAG_0);
			query.eq(SysPermission::getSubSystemId, subSystemId);
...
//	}
	...
	/**
	 * 查询用户拥有的子系统权限（根据TOKEN）
	 * @param token
	 * @return
	 */
	@RequestMapping(value = "/getUserSubSystemByToken", method = RequestMethod.GET)
	public Result<?> getUserSubSystemByToken(@RequestParam(name = "token", required = true) String token) {
		Result<List> result = new Result<>();
		try {
			if (oConvertUtils.isEmpty(token)) {
				return Result.error("TOKEN不允许为空！");
			}
			log.info(" ------ 通过令牌获取用户拥有的子系统权限 ---- TOKEN ------ " + token);
			String username = JwtUtil.getUsername(token);

			List<SysSubSystem> subSystemList = sysPermissionService.querySubSystemByUser(username);
			result.setResult(subSystemList);
		} catch (Exception e) {
			result.error500("查询失败:" + e.getMessage());
			log.error(e.getMessage(), e);
		}
	...
	public Result<?> getUserPermissionByToken(@RequestParam(name = "token", required = true) String token,
											  @RequestParam(name = "subSystemId", required = true) String subSystemId) {
		Result<JSONObject> result = new Result<JSONObject>();
		try {
			if (oConvertUtils.isEmpty(token)) {
				return Result.error("TOKEN不允许为空！");
			}
			if (oConvertUtils.isEmpty(subSystemId)) {
				return Result.ok();
			}
			log.info(" ------ 通过令牌获取用户拥有的访问菜单 ---- TOKEN ------ " + token);
			List<SysPermission> metaList = new ArrayList<>();
			List<SysPermission> allSystemMetaList = new ArrayList<>(); //所有子系统的权限清单
			String username = JwtUtil.getUsername(token);
			...
			if (loginUser != null && loginUser.isSuperAdmin()) {
				...
				query = new LambdaQueryWrapper<SysPermission>();
				query.eq(SysPermission::getDelFlag, CommonConstant.DEL_FLAG_0);
				query.orderByAsc(SysPermission::getSortNo);
				allSystemMetaList = sysPermissionService.list(query);
			} else {
				metaList = sysPermissionService.queryByUser(username, subSystemId);
				allSystemMetaList = sysPermissionService.queryByUser(username, null);
			}
			//添加首页路由
...
//			}
			...
			//查询当前用户是否有首页权限 并且将首页强制到1
			List<SysPermission> sysPermissions = sysPermissionService.queryHomePageByUserName(loginUser.getUsername());
			sysPermissions.addAll(metaList);
			...
			//通过所有子系统的权限获取按钮权限，解决前台不同子系统之间功能查看导致的权限按钮不展示的问题
			this.getAuthJsonArray(authjsonArray, allSystemMetaList);
			//查询所有的权限
			LambdaQueryWrapper<SysPermission> query = new LambdaQueryWrapper<SysPermission>();
			query.eq(SysPermission::getDelFlag, CommonConstant.DEL_FLAG_0);
	 ...
	 * @return
	 */
	@RequestMapping(value = "/getUserAllPermissionByToken", method = RequestMethod.GET)
	public Result<?> getUserAllPermissionByToken(@RequestParam(name = "token", required = true) String token,
											  @RequestParam(name = "subSystemList", required = true) String subSystemList) {
		Result<JSONObject> result = new Result<JSONObject>();
		try {
			if (oConvertUtils.isEmpty(token)) {
				return Result.error("TOKEN不允许为空！");
			}
			if (oConvertUtils.isEmpty(subSystemList)) {
				return Result.ok();
			}
			log.info(" ------ 通过令牌获取用户拥有的访问菜单 ---- TOKEN ------ " + token);
			List<SysPermission> metaList = new ArrayList<>();
			String username = JwtUtil.getUsername(token);
			metaList = sysPermissionService.queryAllPermissionByUser(username, subSystemList);

			//将首页强制性的排到第一的位置
			List<SysPermission> sysPermissions = sysPermissionService.queryHomePage();
			sysPermissions.addAll(metaList);
	...
	@RequestMapping("/queryPadRouters")
	public Result<?> queryPadRouters(String token){
		Result<JSONObject> result = new Result<JSONObject>();
		String username = JwtUtil.getUsername(token);
		List<SysPermission> sysPermissions = sysPermissionService.queryPadRouters(username);
		JSONObject json = new JSONObject();
		JSONArray menujsonArray = new JSONArray();
		this.getPermissionJsonArray(menujsonArray, sysPermissions, null);
		//路由菜单
...
//	@RequiresRoles({ "admin" })
	...
	public Result<Map<String, Object>> queryTreeList(@RequestParam(name="subSystemId") String subSystemId) {
	...
	public Result<List<String>> queryRolePermission(@RequestParam(name = "roleId", required = true) String roleId,
													@RequestParam(name = "subSystemId", required = false) String subSystemId) {
		Result<List<String>> result = new Result<>();
		try {
			List<SysRolePermission> list = sysRolePermissionService.getRolePermission(roleId, subSystemId);
//			List<SysRolePermission> list = sysRolePermissionService.list(new QueryWrapper<SysRolePermission>().lambda().eq(SysRolePermission::getRoleId, roleId));
	 ...
	 * @return
	 */
	@RequestMapping(value = "/queryRoleSubSystem", method = RequestMethod.GET)
	public Result<List<String>> queryRoleSubSystem(@RequestParam(name = "roleId", required = true) String roleId) {
		Result<List<String>> result = new Result<>();
		try {
			List<SysRoleSubSystem> list = sysRoleSubSystemService.list(new QueryWrapper<SysRoleSubSystem>().lambda().eq(SysRoleSubSystem::getRoleId, roleId));
			result.setResult(list.stream().map(SysRoleSubSystem::getSubSystemId).collect(Collectors.toList()));
			result.setSuccess(true);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
		}
...
//	@RequiresRoles({ "admin" })
	public Result<String> saveRolePermission(@RequestBody JSONObject json) {
		...
		try {
			...
			String subSystemIds = json.getString("subSystemIds");
			this.sysRolePermissionService.saveRolePermission(roleId, permissionIds, lastPermissionIds);
			if (!StringUtils.isEmpty(subSystemIds)) {
				//先删除数据
				sysRoleSubSystemService.remove(new LambdaQueryWrapper<SysRoleSubSystem>().eq(SysRoleSubSystem::getRoleId, roleId));
				//保存子系统授权
				LoginUser loginUser = (LoginUser)SecurityUtils.getSubject().getPrincipal();
				List<SysRoleSubSystem> sysRoleSubSystemList = new ArrayList<>();
				for(String subSystemId : subSystemIds.split(",")) {
					SysRoleSubSystem sysRoleSubSystem = new SysRoleSubSystem();
					sysRoleSubSystem.setId(IdUtil.simpleUUID());
					sysRoleSubSystem.setRoleId(roleId);
					sysRoleSubSystem.setSubSystemId(subSystemId);
					sysRoleSubSystem.setCreateBy(loginUser.getUsername());
					sysRoleSubSystem.setCreateTime(new Date());
					sysRoleSubSystemList.add(sysRoleSubSystem);
				}
				sysRoleSubSystemService.saveBatch(sysRoleSubSystemList);
			}
			result.success("保存成功！");
	...
	/**
	 * 判断是否外网URL 例如： http://localhost:8080/cmms/swagger-ui.html#/ 支持特殊格式： {{
