...
public class DuplicateCheckController {
	...
	@ApiOperation("存在校验接口")
	public Result<Object> doDuplicateCheckN(DuplicateCheckVo duplicateCheckVo, HttpServletRequest request) {
		Long num = null;
		...
		if (num == null || num == 0) {
			// 该值不可用
			log.info("该值不可用，系统中不存在！");
			return Result.error("该值不可用，系统中不存在！");
		} else {
			// 该值可用
			return Result.ok("该值可用！");
