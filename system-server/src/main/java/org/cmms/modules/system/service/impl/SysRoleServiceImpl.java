...
import org.cmms.modules.system.mapper.SysRoleMapper;
...
public class SysRoleServiceImpl extends ServiceImpl<SysRoleMapper, SysRole> implements ISysRoleService {
    ...
    @Override
    public boolean isRoleCode(String username) {
        List<SysRole> listByUsername = baseMapper.getListByUsername(username);
        if (CollUtil.isNotEmpty(listByUsername)){
            for (int i = 0; i < listByUsername.size(); i++) {
                SysRole sysRole = listByUsername.get(i);
                if (StringUtils.isNotBlank(sysRole.getRoleCode())){
                    if ("ZHHZ".equals(sysRole.getRoleCode()))
                        return true;
                }
            }
        }
