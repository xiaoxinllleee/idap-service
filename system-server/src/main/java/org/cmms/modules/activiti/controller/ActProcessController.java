...
//import org.cmms.common.utils.SecurityUtil;
import org.cmms.common.vo.PageVo;
import org.cmms.common.vo.Result;
import org.cmms.common.vo.SearchVo;
import org.cmms.modules.activiti.entity.*;
import org.cmms.modules.activiti.service.*;
import org.cmms.modules.activiti.vo.ProcessNodeVo;
/*import org.cmms.modules.base.entity.User;
import org.cmms.modules.base.entity.UserRole;
import org.cmms.modules.base.service.UserRoleService;
...
public class ActProcessController {
        ...
        for(Process process : processes){
            ...
            for(FlowElement element : elements){
                ...
                if(element instanceof StartEvent){
                    ...
                    // 设置关联岗位
                    node.setPosts(actNodeService.findPostByNodeId(element.getId()));
                    // 设置关联部门
                    ...
                    //动态设置类名
                    node.setDynamicClass(actNodeService.findDynamicClassByNodeId(element.getId()));
        ...
        if(!allUser){
            ...
            if(roleIds!=null){
                for(String roleId : roleIds){
                    // 通过角色获取用户
                    List<SysUserRole> userRoles = userRoleService.findByRoleId(roleId);
           ...
           /* if(departmentIds!=null){
                for(String departmentId : departmentIds){
                    // 通过部门获取用户
                    List<SysUser> users = sysUserService.findByDepartmentId(departmentId);
                    for (User u : users){
                        // 保存用户
    ...
    public Result<Object> editNodeUser(@RequestParam String nodeId,
                                       ...
                                       @RequestParam(required = false) String[] postIds,
                                       @RequestParam(required = false) String[] departmentIds,
                                       @RequestParam(required = false) String dynamicClass,
        ...
        if (postIds != null) {
            for (String postId : postIds) {
                ActNode actNode = new ActNode().setNodeId(nodeId).setRelateId(postId).setType(ActivitiConstant.NODE_POST);
                list.add(actNode);
            }
        }
        // 分配新部门
        ...
        if (dynamicClass != null) {
            ActNode actNode = new ActNode().setNodeId(nodeId).setRelateId(dynamicClass).setType(ActivitiConstant.NODE_DYNAMIC);
            list.add(actNode);
        }
        // 批量保存
