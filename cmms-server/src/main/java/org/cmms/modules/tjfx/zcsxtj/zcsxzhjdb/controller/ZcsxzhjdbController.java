package org.cmms.modules.tjfx.zcsxtj.zcsxzhjdb.controller;
...
@Api(tags="整村授信支行进度表")
@RestController
@RequestMapping("/zcsxzhjdb/zcsxzhjdb")
public class ZcsxzhjdbController extends JeecgController<Zcsxzhjdb, IZcsxzhjdbService> {
	@Autowired
	private IZcsxzhjdbService zcsxzhjdbService;
	@Autowired
	private IZcsxTjfwService zcsxTjfwService;
	@Autowired
	private ZcsxzhjdbMapper zcsxzhjdbMapper;
	 ...
	 * @return
	 */
	@AutoLog(value = "整村授信支行进度表-分页列表查询")
	@ApiOperation(value="整村授信支行进度表-分页列表查询", notes="整村授信支行进度表-分页列表查询")
	@GetMapping(value = "/list")
	public Result<?> queryPageList(Zcsxzhjdb zcsxzhjdb,
								   @RequestParam(name="pageNo", defaultValue="1") Integer pageNo,
								   @RequestParam(name="pageSize", defaultValue="10") Integer pageSize,
								   HttpServletRequest req,String tjnf,String pc) {
		QueryWrapper<Zcsxzhjdb> queryWrapper = QueryGenerator.initQueryWrapper(zcsxzhjdb, req.getParameterMap());
		//统计范围
		if (StringUtils.isNotBlank(tjnf) && StringUtils.isNotBlank(pc)){
			queryWrapper.eq("nf", tjnf);
			queryWrapper.eq("pc", pc);
		} else {
			//查询汇总数
			queryWrapper.isNull("nf");
		}
		Page<Zcsxzhjdb> page = new Page<Zcsxzhjdb>(pageNo, pageSize);
		IPage<Zcsxzhjdb> pageList = zcsxzhjdbService.page(page, queryWrapper);
		return Result.ok(pageList);
	}
	 @GetMapping(value = "/maxDate")
	 public Result<?> maxDate(){
		 String substring = zcsxzhjdbMapper.maxDate();
	 ...
	 * @return
	 */
	@AutoLog(value = "整村授信支行进度表-添加")
	@ApiOperation(value="整村授信支行进度表-添加", notes="整村授信支行进度表-添加")
	@PostMapping(value = "/add")
	public Result<?> add(@RequestBody Zcsxzhjdb zcsxzhjdb) {
		zcsxzhjdbService.save(zcsxzhjdb);
	 ...
	 * @return
	 */
	@AutoLog(value = "整村授信支行进度表-编辑")
	@ApiOperation(value="整村授信支行进度表-编辑", notes="整村授信支行进度表-编辑")
	@PutMapping(value = "/edit")
	public Result<?> edit(@RequestBody Zcsxzhjdb zcsxzhjdb) {
		zcsxzhjdbService.updateById(zcsxzhjdb);
	 ...
	 * @return
	 */
	@AutoLog(value = "整村授信支行进度表-通过id删除")
	@ApiOperation(value="整村授信支行进度表-通过id删除", notes="整村授信支行进度表-通过id删除")
	@DeleteMapping(value = "/delete")
	public Result<?> delete(@RequestParam(name="id",required=true) String id) {
		zcsxzhjdbService.removeById(id);
	 ...
	 * @return
	 */
	@AutoLog(value = "整村授信支行进度表-批量删除")
	@ApiOperation(value="整村授信支行进度表-批量删除", notes="整村授信支行进度表-批量删除")
	@DeleteMapping(value = "/deleteBatch")
	public Result<?> deleteBatch(@RequestParam(name="ids",required=true) String ids) {
		this.zcsxzhjdbService.removeByIds(Arrays.asList(ids.split(",")));
	 ...
	 * @return
	 */
	@AutoLog(value = "整村授信支行进度表-通过id查询")
	@ApiOperation(value="整村授信支行进度表-通过id查询", notes="整村授信支行进度表-通过id查询")
	@GetMapping(value = "/queryById")
	public Result<?> queryById(@RequestParam(name="id",required=true) String id) {
		Zcsxzhjdb zcsxzhjdb = zcsxzhjdbService.getById(id);
		return Result.ok(zcsxzhjdb);
	}
  ...
  public ModelAndView exportXls(HttpServletRequest request, Zcsxzhjdb zcsxzhjdb, String tjnf,String pc) {
	  // Step.1 组装查询条件
	  QueryWrapper<Zcsxzhjdb> queryWrapper = QueryGenerator.initQueryWrapper(zcsxzhjdb, request.getParameterMap());
	  //统计范围
	  if (StringUtils.isNotBlank(tjnf) && StringUtils.isNotBlank(pc)){
		  queryWrapper.eq("nf", tjnf);
		  queryWrapper.eq("pc", pc);
	  ...
	  List<Zcsxzhjdb> exportList = service.list(queryWrapper);
	  //金额由元变成万元
	  ...
	  String title = "整村授信支行进度表";
	  ModelAndView mv = new ModelAndView(new JeecgEntityExcelView());
	  mv.addObject(NormalExcelConstants.FILE_NAME, title); //此处设置的filename无效 ,前端会重更新设置一下
	  mv.addObject(NormalExcelConstants.CLASS, Zcsxzhjdb.class);
  ...
  public Result<?> importExcel(HttpServletRequest request, HttpServletResponse response) {
      return super.importExcel(request, response, Zcsxzhjdb.class);
  }
