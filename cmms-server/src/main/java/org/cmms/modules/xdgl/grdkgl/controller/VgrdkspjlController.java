...
@Api(tags = "个人贷款审批记录")
@RestController
@RequestMapping("/xdgl/grdkgl/vgrdkspjl")
public class VgrdkspjlController extends JeecgController<Vgrdkspjl, IVgrdkspjlService> {
    @Autowired
...
//        List<Vgrdkspjl> list = null;
//        if (vgrdkspjl.getZjhm()==null  && vgrdkspjl.getKhmc()==null){
//             list = iVGrdkSpjlService.queryDksp(vgrdkspjl.getSskhjl());
//        }else if(vgrdkspjl.getZjhm()==null  && vgrdkspjl.getKhmc()!=null){
//             list = iVGrdkSpjlService.selectByKhmc(vgrdkspjl.getSskhjl(),vgrdkspjl.getKhmc());
//        }else if(vgrdkspjl.getZjhm()!=null  && vgrdkspjl.getKhmc()==null){
//            list = iVGrdkSpjlService.selectByZjhm(vgrdkspjl.getSskhjl(),vgrdkspjl.getZjhm());
//        }else {
//            list = iVGrdkSpjlService.queryByZjhm(vgrdkspjl.getSskhjl(),vgrdkspjl.getZjhm(),vgrdkspjl.getKhmc());
//        }
//        return Result.ok(list);

//        select * from V_CAMS_ZCSX_GRXDCJXX_SPJL where  status =2 and jtspzzzt = 2
//        and (sskhjl = #{userId} or id in (select id from CAMS_ZCSX_GRXDJTSPXZ where zrrid = #{userId}))
        QueryWrapper<Vgrdkspjl> queryWrapper = QueryGenerator.initQueryWrapper(vgrdkspjl, req.getParameterMap());
        queryWrapper.orderByDesc("create_time");
        queryWrapper.eq("status", 2);
        queryWrapper.eq("jtspzzzt", 2);
        queryWrapper.eq("sskhjl", vgrdkspjl.getSskhjl()).or().inSql("id", "select id from CAMS_ZCSX_GRXDJTSPXZ where zrrid = " + vgrdkspjl.getSskhjl() + "");
        if (vgrdkspjl.getZjhm() == null && vgrdkspjl.getKhmc() != null) {
